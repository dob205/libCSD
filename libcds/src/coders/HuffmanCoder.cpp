...
HuffmanCoder::~HuffmanCoder() { freeHuff(huff_table); }

size_t HuffmanCoder::maxLength() const { return huff_table.depth; }
...
void HuffmanCoder::save(std::ostream &fp) const {
  saveValue<uint>(fp, HUFF_HDR);
  saveHuff(huff_table, fp);
}

HuffmanCoder *HuffmanCoder::load(std::istream &fp) {
