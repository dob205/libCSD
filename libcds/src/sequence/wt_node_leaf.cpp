...
size_t wt_node_leaf::rank(uint symbol, size_t pos, uint, wt_coder *) const {
...
size_t wt_node_leaf::select(uint symbol, size_t pos, uint, wt_coder *) const {
  if (symbol != this->symbol)
    return (size_t)-1;
  if (pos == 0 || pos > count)
    return (size_t)-1;
...
uint wt_node_leaf::access(size_t) const { return symbol; }
