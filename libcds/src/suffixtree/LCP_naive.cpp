...
LCP_naive::LCP_naive(TextIndex *csa, char *text, size_t n) {
  ...
  for (size_t i = 1; i < length; i++) {
    val = csa->getSA(i);
    act = n - val;
    set_field_64(lcp_array, b, i,
                 lcp_length(text, n - prev, val, min(prev, act)));
    prev = act;
  }
}

size_t LCP_naive::get_LCP(size_t i, TextIndex *) const {
  return get_field_64(lcp_array, b, i);
}

size_t LCP_naive::get_seq_LCP(size_t i, TextIndex *csa, size_t **, size_t *,
                              bool) const {
