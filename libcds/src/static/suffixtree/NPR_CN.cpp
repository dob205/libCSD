...
size_t NPR_CN::find_NSV_level_r(size_t value_v, size_t next_pos,
                                size_t r) const {
  size_t lim;
...
size_t NPR_CN::find_PSV_level_r(size_t value_v, size_t next_pos,
                                size_t r) const {
  size_t lim;
...
NPR_CN *NPR_CN::load(ifstream &fp) {
  ...
  for (size_t i = 0; i < npr->l; i++)
    npr->min_pos[i] =
        loadValue<size_t>(fp, (npr->level_size[i] * npr->bits_b + W - 1) / W);
  return npr;
}
