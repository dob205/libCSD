...
private:
  ...
  size_t **min_level; // array for each level that contain the min value of each
                      // block
  size_t **min_pos;   // array for each level that contain the local position of
                      // the min value of each block
  size_t *level_size; // size of each level

public:
  /*NPR Data structure base on Canovas and Navarro "Practical Compressed Suffix
   * Trees"
  ...
  // Return the left-most position  j, x<= j <= y  , where is the minimun in the
  // range [x,y]
  ...
  static NPR_CN *load(std::ifstream &fp);
};
