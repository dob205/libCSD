...
protected:
  ...
  DTsucc **M; // table M for the out-of-block queries (contains indices of
              // block-minima)

  uint M_depth;      // depth of table M
  uint **Mprime;     // table M' for superblock-queries (contains indices of
                     // block-minima)
  ...
  inline uint m(uint k, uint block) { return M[k][block] + (block * sprime); }
  ...
  // stuff for clearing the least significant x bits (change for 64-bit
  // computing)
  static constexpr DTsucc HighestBitsSet[8] = {0xff, 0xfe, 0xfc, 0xf8,
                                               0xf0, 0xe0, 0xc0, 0x80};
