...
unsigned int RMQ_succinct::lsb(DTsucc v) { return LSBTable256[v]; }
...
unsigned int RMQ_succinct::query(unsigned int i, unsigned int j) {
  ...
  if (mb_i == mb_j) { // only one microblock-query
    ...
    min = min_i == 0 ? s_mi + s - 1
                     : s_mi + lsb(min_i); // left in-microblock-query
    min_j =
    ...
    if (mb_j > mb_i + 1) {              // otherwise we're done!
      ...
      if (j >= s_bj + s) { // and yet another microblock-query!
        ...
        min_j =
            Prec[type[mb_j]][s - 1] == 0
                ? s_mj - 1
                : s_bj + lsb(Prec[type[mb_j]][s - 1]); // right in-block-query
        if (a[min_j] < a[min])
          min = min_j;
      }
      ...
      if (block_difference > 1) { // otherwise we're done!
        unsigned int k, twotothek,
            block_tmp; // for index calculations in M and M'
        ...
        if (s_bj - s_bi - sprime <=
          ...
          block_tmp =
              block((sb_i + 1) * sprimeprime); // end of left out-of-block-query
          k = log2fast(block_tmp - b_i);
          twotothek = 1 << k; // 2^k
          i = m(k, b_i);
          j = m(k, block_tmp + 1 - twotothek);
          min_i = a[i] <= a[j] ? i : j;

          block_tmp =
              block(sb_j * sprimeprime); // start of right out-of-block-query
          k = log2fast(b_j - block_tmp);
          twotothek = 1 << k; // 2^k
          block_tmp--; // going one block to the left doesn't harm and saves
                       // some tests
          i = m(k, block_tmp);
          j = m(k, b_j - twotothek);
          min_j = a[i] <= a[j] ? i : j;
...
RMQ_succinct::RMQ_succinct(int *a, unsigned int n) {
  ...
  // prec[i]: the jth bit is 1 iff j is 1. pos. to the left of i where a[j] <
  // a[i]
  ...
  for (unsigned int j = 1; j < M_depth; j++) {
    M[j] = new DTsucc[nb];
    for (unsigned int i = 0; i < nb - dist;
         i++) { // be careful: loop may go too far
      M[j][i] = a[m(j - 1, i)] <= a[m(j - 1, i + dist)]
                    ? M[j - 1][i]
                    : M[j - 1][i + dist] +
                          (dist * sprime); // add 'skipped' elements in a
    }
    for (unsigned int i = nb - dist; i < nb; i++)
  ...
  for (unsigned int j = 1; j < Mprime_depth; j++) {
    ...
    for (unsigned int i = nsb - dist; i < nsb; i++)
