...
uint *LCP::create_lcp(TextIndex *csa, char *text, uint n, int q) const {
  ...
  int *sa = NULL;
  int *plcp = NULL;
  /*q==-1 its mean that for any position we just use psi for calculate each
   * LCP*/
  if (q != -1) {
    m = 1 + (n - 1) / q;
  ...
  for (k = 1; k < (int)n; k++) {
    ...
    if (sa_k % q == 0)
      ...
      while (sa_k + h < n && sa_ant + h < n &&
             text[sa_k + h] == text[sa_ant + h])
        h++;
      lcp[k] = h;
    }
  }
...
LCP *LCP::load(ifstream &fp) {
  ...
  case SAD_GON_OS:
    return LCP_Sad::load(fp);
  case FMN_RRR_OS:
    return LCP_FMN::load(fp);
  case PT:
    return LCP_PT::load(fp);
  case PHI:
    return LCP_PhiSpare::load(fp);
  case DAC:
    return LCP_DAC::load(fp);
  case DAC_VAR:
    return LCP_DAC_VAR::load(fp);
  }
  return NULL;
}
} // namespace cds_static
