...
void LCP_DAC_VAR::calc_kvalues(uint max_lcp, uint *frec_acu, ushort **kvalues,
                               uint *nkvalues) const {
  // uint list_length = 0;
  uint nBits;
  ...
  nBits = bits(max_lcp);
  // This table will contain the size of the best option for store the firts x
  // bits
  ...
  for (uint i = 1; i <= nBits; i++) {
    ...
    for (j = 0; j < i; j++) {
      ...
      if (j == 0)
        currentSize =
            tableSize[j] +
            ((ulong)(frec_acu[max_lcp] - frec_acu[posVocInf])) * ((i - j));
      else
        currentSize =
            tableSize[j] +
            ((ulong)(frec_acu[max_lcp] - frec_acu[posVocInf])) * ((i - j) + 1) +
            (frec_acu[max_lcp] - frec_acu[posVocInf]) / FACT_RANK;
  ...
  for (j = 0; j < tableNLevels[nBits]; j++) {
    ...
    if (j == tableNLevels[nBits])
      sumaTotal +=
          ((ulong)(frec_acu[max_lcp] - frec_acu[posVocInf])) * bitsCount;
    else
      sumaTotal +=
          ((ulong)(frec_acu[max_lcp] - frec_acu[posVocInf])) * (bitsCount + 1) +
          (frec_acu[max_lcp] - frec_acu[posVocInf]) / FACT_RANK;
...
size_t LCP_DAC_VAR::get_LCP(size_t i, TextIndex *) const {
  return (size_t)rep->access(i + 1);
}

size_t LCP_DAC_VAR::get_seq_LCP(size_t i, TextIndex *, size_t **next_pos,
