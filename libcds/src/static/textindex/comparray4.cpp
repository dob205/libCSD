...
static inline int getbitD(unsigned short *B, int i) {
  int j, l, x;
  i--;
  j = i >> 4;
  l = i & (DD - 1);
...
static int getbit(unsigned short *B, int i) {
  int j, l;
  i--;
  j = i >> 4;
  l = i & (DD - 1);
...
static int setbit(unsigned short *B, int i, int x) {
  int j, l;
  i--;
  j = i / DD;
  l = i % DD;
...
static int encodegamma(unsigned short *B, int p, int x) /* x ��1�ȏ� */
{
  int j, w;
  if (x <= 0) {
...
static int decodegamma(unsigned short *B, int p, int *ans) {
  int w, x;
  int w2;
#if 0
		x = getbitD(B,1+p);
...
static inline int psi_list(CSA *SA, int i) {
  int j, l, r, m;
#ifdef DEBUG
  if (i > SA->n || i < 1) {
...
static void psisort2(int *p, int *I, unsigned char *s, int n) {
  int i, sum;
  int C[SIGMA];
  int x, c;
  for (i = 0; i < SIGMA; i++)
...
static void writeint(int x, FILE *f) {
  int tmp;
  tmp = x;
...
static int readint(FILE *f) {
  int tmp;
  int bytes_read = fread(&tmp, sizeof(int), 1, f);
...
int np;
static int csa_lookup2(CSA *SA, int i) {
  int v, two;
  v = 0;
  two = SA->two;
  while (i % two != 0) {
...
/*
static int *csa_batchlookup3(CSA *SA, int l, int r, int len) {
  int *I;
  int *P;
  int v;
  int m;
