...
static void parse_parameters(char *options, int *num_parameters,
  ...
  char *options_temp;
  while (i < len_options) {
    while ((i < len_options) && is_delimeter(delimiters, options[i]))
      i++;
    temp = i;
    while ((i < len_options) && !is_delimeter(delimiters, options[i]))
      i++;
...
static int load_index(char *filename, void **index) {
  char fname1[256 + 4] = "", fname2[256 + 4] = "";
  CSA *SA = NULL;
...
static inline int index_size(void *index, ulong *size) {
  CSA *SA = (CSA *)index;
  *size = 0;
#ifdef USE_MMAP
  *size += sizeof(MMAP) * 2;
...
static inline int index_size_count(void *index, ulong *size) {
  CSA *SA = (CSA *)index;
  *size = 0;
#ifdef USE_MMAP
  *size += sizeof(MMAP) * 2;
...
static inline int index_size_locate(void *index, ulong *size) {
  CSA *SA = (CSA *)index;
  *size = 0;
#ifdef USE_MMAP
  *size += sizeof(MMAP) * 2;
...
//Querying the Index//
////////////////////*/
static inline int count(void *index, uchar *pattern, ulong length,
                        ulong *numocc) {
  int l, r;
  CSA *SA = (CSA *)index;
  (void)csa_bsearch(pattern, length, SA, &l, &r);
  *numocc = r - l + 1;
  return 0;
}
