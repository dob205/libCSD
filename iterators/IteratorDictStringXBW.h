 ...
 *
 * Iterator class for scanning strings (continuous range) in a XBW
 * representation.
 *
...
public:
   ...
   */
  IteratorDictStringXBW(uchar *prefix, uint prefixLen, size_t left,
                        size_t right, XBW *xbw, uint maxlength) {
...
protected:
  ...
  void idToStr(uint id, uint cnt) {
    if (id == processed) {
      pos = strLen;
      if (cnt > 0)
        str[pos] = xbw->unmap[xbw->alpha->access(id)];
      else
        str[pos] = 0;
