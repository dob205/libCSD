...
public:
  ...
  */
  IteratorDictStringRPDAC(LogSequence *G, uint64_t terminals, DAC_VLS *C,
                          size_t offset, size_t scanneable, uint maxlength) {
    this->G = G;
  ...
  */
  unsigned char *next(uint *strLen) {
    ...
    for (uint j = 0; j < len; j++) {
      if (rules[j] >= terminals)
        expandRule(rules[j] - terminals);
      else {
  ...
  /** Generic destructor. */
  ~IteratorDictStringRPDAC() { delete[] strCurr; }
...
protected:
  ...
  void expandRule(uint rule) {
    uint left = G->getField(2 * rule);
    uint right = G->getField((2 * rule) + 1);

    if (left >= terminals)
      expandRule(left - terminals);
    else {
      strCurr[lenCurr] = (char)left;
      lenCurr++;
    }

    if (right >= terminals)
      expandRule(right - terminals);
    else {
      strCurr[lenCurr] = (char)right;
      lenCurr++;
    }
  }
};
