...
public:
  ...
  */
  IteratorDictStringRPFC(RePair *rp, uint bitsrp, uchar *ptr, uint offset,
  ...
  */
  ...
  /** Generic destructor. */
  ~IteratorDictStringRPFC() { delete[] strCurr; }
...
protected:
  ...
  inline void decodeNext() {
    ...
    while (strCurr[lenCurr - 1] != rp->maxchar) {
      ...
      if (rule >= rp->terminals)
        lenCurr += rp->expandRule(rule - rp->terminals, (strCurr + lenCurr));
      else {
