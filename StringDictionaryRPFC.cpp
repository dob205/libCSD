...
StringDictionaryRPFC::StringDictionaryRPFC(IteratorDictString *it,
                                           uint bucketsize) {
  ...
  // 3) Compressing the dictionary
  {
    ...
    for (bucket = 1; bucket <= buckets; bucket++) {
      ...
      // Updating the ptr value to the beginning of the corresponding internal
      // string
      ...
      for (; ptrB <= ptrE; ptrB++)
        bytesStrings += encodeSymbol(intStrings[ptrB],
                                     &(textStrings[bytesStrings]), &offset);
...
uchar *StringDictionaryRPFC::extract(size_t id, uint *strLen) {
  if ((id > 0) && (id <= elements)) {
...
IteratorDictID *StringDictionaryRPFC::locatePrefix(uchar *str, uint strLen) {
  ...
  if (leftBucket > NORESULT) {
    ...
    if (leftBucket == rightBucket) {
      ...
      if (leftID == NORESULT)
        ...
        rightID = leftID +
                  searchDistinctPrefix(ptr, scanneable - leftID + 1, decoded,
                                       &decLen, str, strLen, &offset) -
                  1;
...

      {
        ...
        offset = 0;
        rightID = searchDistinctPrefix(ptr, scanneable, decoded, &decLen, str,
                                       strLen, &offset);
        rightID += (rightBucket - 1) * bucketsize;
      }
    }
...
uint StringDictionaryRPFC::locateRank(uint rank) { return rank; }

IteratorDictString *StringDictionaryRPFC::extractPrefix(uchar *str,
                                                        uint strLen) {
  ...
  if (left != NORESULT) {
    ...
    return new IteratorDictStringRPFC(rp, bitsrp, textStrings + ptrS, leftpos,
                                      bucketsize, right - left + 1, maxlength);
  } else
    return NULL;
}
...
uchar *StringDictionaryRPFC::extractRank(uint rank, uint *strLen) {
  return extract(rank, strLen);
}

IteratorDictString *StringDictionaryRPFC::extractTable() {
  return new IteratorDictStringRPFC(rp, bitsrp, textStrings, 0, bucketsize,
                                    elements, maxlength);
}

size_t StringDictionaryRPFC::getSize() {
  return bytesStrings * sizeof(uchar) + blStrings->getSize() + rp->getSize() +
         sizeof(StringDictionaryRPFC);
}

void StringDictionaryRPFC::save(std::ostream &out) {
...
StringDictionary *StringDictionaryRPFC::load(std::istream &in) {
...
inline uchar *StringDictionaryRPFC::getHeader(size_t idbucket, uchar **str,
                                              uint *strLen) {
...
bool StringDictionaryRPFC::locateBucket(uchar *str, size_t *idbucket) {
...
void StringDictionaryRPFC::locateBoundaryBuckets(uchar *str, uint strLen,
...
uint StringDictionaryRPFC::searchPrefix(uchar **ptr, uint scanneable,
                                        uchar *decoded, uint *decLen,
                                        uchar *str, uint strLen, uint *offset) {
  uint id = NORESULT;
...
uint StringDictionaryRPFC::searchDistinctPrefix(uchar *ptr, uint scanneable,
  ...
  for (id = 1; id < scanneable; id++)
    if (decodeString(decoded, decLen, &ptr, offset) < strLen)
      break;

  return id;
}

inline uint StringDictionaryRPFC::encodeSymbol(uint symbol, uchar *text,
                                               uint *offset) {
  uint processed = 0;
  uint bytes = 0;
...
inline uint StringDictionaryRPFC::decodeSymbol(uint *symbol, uchar *ptr,
                                               uint *offset) {
  *symbol = 0;
  ...
  while ((bitsrp - processed) >= (8 - (*offset))) {
    *symbol =
        (*symbol << (8 - (*offset))) | (*(ptr + bytes) & mask(8 - (*offset)));
  ...
  if (bitsrp > processed) {
    *offset = bitsrp - processed;
    *symbol =
        (*symbol << (bitsrp - processed)) | (*(ptr + bytes) >> (8 - *offset));
  }

  return bytes;
}

inline uint StringDictionaryRPFC::decodeString(uchar *str, uint *strLen,
                                               uchar **ptr, uint *offset) {
