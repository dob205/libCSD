...
void Heap::destroyHeap(Theap *H) {
  ...
  //    Thfreq *l,*n;
  for (i = 1; i < H->sqrtu; i++)
    ArrayG::destroyArray(&H->infreq[i]);
  free(H->infreq);
  H->infreq = NULL;
  free(H->freq);
  H->freq = NULL;
...
void Heap::move(Tarray A, int i, int j, Trecord *rec) {
  int id = A.pairs[j];
  A.pairs[i] = id;
  rec[id].hpos = i;
}
...
void Heap::incFreq(Theap *H, int id) {
  Trecord *rec = H->Rec->records;
  Thnode *p;
  Thfreq *f, *lf;
  int fp, lfp;

  if ((rec[id].pair.left == 0) || (rec[id].pair.right == 0))
    return;
...
void Heap::decFreq(Theap *H, int id) {
  Trecord *rec = H->Rec->records;
  int freq = rec[id].freq--;
  int hpos = rec[id].hpos;
  Thnode *p;
...
void Heap::insertHeap(Theap *H, int id) {
  Trecord *rec = H->Rec->records;
  rec[id].hpos = ArrayG::insertArray(&H->infreq[1], id);
  rec[id].freq = 1;
}

int Heap::extractMax(Theap *H) {
  //***    Trecord *rec = H->Rec->records;
  int ret;
  Thnode *p;
  Thfreq *f;
...
void Heap::heapRepos(Theap *H, int id) {
  Trecord *rec = H->Rec->records;
