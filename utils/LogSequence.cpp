...
void LogSequence::setField(size_t position, size_t value) {
  if (position > numentries) {
    throw "Trying to modify a position out of the structure capacity. Use "
          "push_back() instead";
  }
  if (value > maxval) {
    throw "Trying to insert a value bigger that expected. Please increase "
          "numbits when creating the data structure.";
  }
