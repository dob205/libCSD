...
StringDictionaryRPHTFC::StringDictionaryRPHTFC(IteratorDictString *it,
  ...
  while (bucket <= buckets) {
    // Recollecting statistics for the headers and extracting them for Hu-Tucker
    // compression
  ...
  // 3) Compressing the dictionary and building the decoding table
  {
    ...
    for (bucket = 1; bucket <= buckets; bucket++) {
      ...
      do {
        ...
        builderHT->insertDecodeableSubstr(symbol, &codeSubstr, &ptrSubstr,
      ...
      if (textSubstr.size() > 0) {
        ...
        if (ptrSubstr > TABLEBITSO) {
          ...
          if ((bucket == buckets) && (elements % bucketsize == 0)) {
            ...
            // The first symbol encoding the internal strings is enough for
            // padding because it uses, at least, 16 bits
...
uchar *StringDictionaryRPHTFC::extract(size_t id, uint *strLen) {
  if ((id > 0) && (id <= elements)) {
  ...
  if (leftBucket > NORESULT) {
    ...
    if (leftBucket == rightBucket) {
      ...
      if (leftID == NORESULT)
        ...
        rightID = leftID +
                  searchDistinctPrefix(&c, scanneable - leftID + 1, str, strLen,
                                       &offset) -
                  1;
...
IteratorDictString *StringDictionaryRPHTFC::extractPrefix(uchar *str,
  ...
  if (left != NORESULT) {
    ...
    return new IteratorDictStringRPHTFC(
        tableHT, codewordsHT, rp, bitsrp, textStrings, blStrings, leftbucket,
...
uchar *StringDictionaryRPHTFC::extractRank(uint rank, uint *strLen) {
  return extract(rank, strLen);
}

IteratorDictString *StringDictionaryRPHTFC::extractTable() {
  return new IteratorDictStringRPHTFC(tableHT, codewordsHT, rp, bitsrp,
...
uint StringDictionaryRPHTFC::searchDistinctPrefix(ChunkScan *c, uint scanneable,
  ...
  for (id = 1; id < scanneable; id++)
    if (decodeString(c->str, &c->strLen, &c->b_ptr, offset) < strLen)
      break;

  return id;
}

uint StringDictionaryRPHTFC::encodeSymbol(uint symbol, uchar *text,
                                          uint *offset) {
  uint processed = 0;
  uint bytes = 0;
...
inline uint StringDictionaryRPHTFC::decodeSymbol(uint *symbol, uchar *ptr,
                                                 uint *offset) {
  *symbol = 0;
...
inline uint StringDictionaryRPHTFC::decodeString(uchar *str, uint *strLen,
