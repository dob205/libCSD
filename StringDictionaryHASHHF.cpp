...
StringDictionaryHASHHF::StringDictionaryHASHHF(IteratorDictString *it, uint len,
                                               int overhead) {
  this->type = HASHHF;
  this->elements = 0;
  this->maxlength = 0;
      ...
      if (ptrSubstr > TABLEBITSO) {
        ...
        while (TABLEBITSO > ptrSubstr) {
          ...
          if ((bits + ptrSubstr) <= TABLEBITSO) {
            ...
            codeSubstr =
                (codeSubstr << remaining) | (codeword >> (bits - remaining));
            ptrSubstr = TABLEBITSO;
          }
        }
      }
...
uchar *StringDictionaryHASHHF::extract(size_t id, uint *strLen) {
  if ((id > 0) && (id <= elements)) {
...
IteratorDictID *StringDictionaryHASHHF::locatePrefix(uchar *str, uint strLen) {
  cout << "This dictionary does not provide prefix location" << endl;
  return NULL;
}

IteratorDictID *StringDictionaryHASHHF::locateSubstr(uchar *str, uint strLen) {
  cout << "This dictionary does not provide substring location" << endl;
  return NULL;
}

uint StringDictionaryHASHHF::locateRank(uint rank) {
  cout << "This dictionary does not provide rank location" << endl;
  return 0;
}

IteratorDictString *StringDictionaryHASHHF::extractPrefix(uchar *str,
                                                          uint strLen) {
  cout << "This dictionary does not provide prefix extraction" << endl;
  return NULL;
}

IteratorDictString *StringDictionaryHASHHF::extractSubstr(uchar *str,
                                                          uint strLen) {
  cout << "This dictionary does not provide substring extraction" << endl;
  return NULL;
}

uchar *StringDictionaryHASHHF::extractRank(uint rank, uint *strLen) {
...
size_t StringDictionaryHASHHF::getSize() {
  return bytesStrings * sizeof(uchar) + hash->getSize() +
...
StringDictionary *StringDictionaryHASHHF::load(ifstream &in, uint technique) {
  ...
  if (type != HASHHF)
    return NULL;
  if ((technique != HASHUFF) && (technique != HASHBHUFF) &&
      (technique != HASHBBHUFF))
    return NULL;
