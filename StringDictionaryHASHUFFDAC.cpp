...
StringDictionaryHASHUFFDAC::StringDictionaryHASHUFFDAC(IteratorDictString *it,
  ...
  for (uint i = 1; i < nLevels; i++)
    xLevels[i] = xLevels[i - 1] + levelsIndex[i - 1];
  ...
  for (uint current = 1; current <= elements; current++) {
...

    {
      ...
      for (uint i = 0; i < bytes; i++) {
        ...
        if ((i + 1) < bytes) {
          bS->setBit(xLevels[i], true);
          rankLevels[i]++;
        } else
          bS->setBit(xLevels[i], false);
...
uchar *StringDictionaryHASHUFFDAC::extract(size_t id, uint *strLen) {
  if ((id > 0) && (id <= elements))
    return extractString(id, strLen);
  else {
    *strLen = 0;
    return NULL;
  }
...
size_t StringDictionaryHASHUFFDAC::getSize() {
  return dac->getSize() + hash->getSize() + 256 * sizeof(Codeword) +
         table->getSize() + sizeof(StringDictionaryHASHUFFDAC) +
         256 * sizeof(bool);
}
