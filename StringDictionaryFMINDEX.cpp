...
uchar *StringDictionaryFMINDEX::extract(size_t id, uint *strLen) {
  if ((id > 0) && (id <= elements)) {
    size_t i;
...
uchar *StringDictionaryFMINDEX::extractRank(uint rank, uint *strLen) {
...
void StringDictionaryFMINDEX::build_ssa(uchar *text, size_t len,
  ...
  if (BWTsampling > 0) {
    ...
    for (uint i = 0; i < samples; i++)
      fm_index->suff_sample[i] = separators->rank1(fm_index->suff_sample[i]);
  }
}
