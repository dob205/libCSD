...
unsigned long StringDictionaryXBW::locate(uchar *str, uint strLen) {
  ...
  // The last child (right) is the required string if it is ended by
  // 'maxLabel'...
  if ((left > right) || (xbw->alpha->access(right) != xbw->maxLabel))
    return NORESULT;
  else
    return xbw->alpha->rank(xbw->maxLabel, right);
}

uchar *StringDictionaryXBW::extract(size_t id, uint *strLen) {
  if ((id > 0) && (id <= elements)) {
    uchar *str;
...
IteratorDictID *StringDictionaryXBW::locatePrefix(uchar *str, uint strLen) {
  uchar *qry = new uchar[strLen + 1];
  qry[0] = 0;
  strncpy((char *)qry + 1, (char *)str, strLen);
  ...
  if (left <= right)
    return new IteratorDictIDXBW(left, right, xbw);
  else
...
IteratorDictID *StringDictionaryXBW::locateSubstr(uchar *str, uint strLen) {
  uint left, right;
  xbw->subPathSearch(str, strLen, &left, &right);

  if (left <= right)
    return new IteratorDictIDXBWDuplicates(left, right, xbw);
  else
    return new IteratorDictIDContiguous(NORESULT, NORESULT);
}

uint StringDictionaryXBW::locateRank(uint rank) { return rank; }

IteratorDictString *StringDictionaryXBW::extractPrefix(uchar *str,
                                                       uint strLen) {
...
IteratorDictString *StringDictionaryXBW::extractSubstr(uchar *str,
                                                       uint strLen) {
  uint left, right;
  xbw->subPathSearch(str, strLen, &left, &right);

  if (left <= right)
    return new IteratorDictStringXBWDuplicates(left, right, xbw, maxlength);
  else
    return NULL;
}

uchar *StringDictionaryXBW::extractRank(uint rank, uint *strLen) {
...
size_t StringDictionaryXBW::getSize() {
