...
StringDictionaryRPDAC::StringDictionaryRPDAC(IteratorDictString *it) {
  ...
  rp->Cdac =
      new DAC_VLS(cdict, ic - 2, bits(rp->rules + rp->terminals), maxseq);
...
uchar *StringDictionaryRPDAC::extract(size_t id, uint *strLen) {
  if ((id > 0) && (id <= elements)) {
    uint *rules;
...
IteratorDictID *StringDictionaryRPDAC::locatePrefix(uchar *str, uint strLen) {
  // Binary search comparing s with respect to the corresponding
  // pivot rule.
  size_t left = 1, right = elements, center = 0;
  int cmp = 0;
...
uint StringDictionaryRPDAC::locateRank(uint rank) { return rank; }

IteratorDictString *StringDictionaryRPDAC::extractPrefix(uchar *str,
                                                         uint strLen) {
  ...
  delete it;
  return new IteratorDictStringRPDAC(rp->G, rp->terminals, rp->Cdac, offset,
...
uchar *StringDictionaryRPDAC::extractRank(uint rank, uint *strLen) {
  return extract(rank, strLen);
}

IteratorDictString *StringDictionaryRPDAC::extractTable() {
  return new IteratorDictStringRPDAC(rp->G, rp->terminals, rp->Cdac, 0,
                                     elements, maxlength);
}

size_t StringDictionaryRPDAC::getSize() {
...
StringDictionaryRPDAC::~StringDictionaryRPDAC() { delete rp; }
