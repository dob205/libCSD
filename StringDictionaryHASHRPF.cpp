...
unsigned long StringDictionaryHASHRPF::locate(uchar *str, uint strLen) {
  ...
  for (unsigned long i = 1; i < hash->tsize; i++) {
    ...
    if (rp->extractStringAndCompareRP(hash->getValuePos(next), str, strLen) ==
        0)
...
uchar *StringDictionaryHASHRPF::extract(size_t id, uint *strLen) {
  if ((id > 0) && (id <= elements)) {
    ...
    *strLen = 0;
    ...
    while (true) {
      rule = rp->Cls->getField(position + ptr);
      ptr++;

      if (rule >= rp->terminals)
        (*strLen) += rp->expandRule(rule - rp->terminals, (s + (*strLen)));
      else {
        s[*strLen] = (uchar)rule;
        (*strLen)++;
      }

      if (s[*strLen - 1] == rp->maxchar)
        break;
    }
...
StringDictionary *StringDictionaryHASHRPF::load(std::ifstream &in,
  ...
  if (type != HASHRPF)
    return NULL;
  if ((technique != HASHRP) && (technique != HASHBRP) &&
      (technique != HASHBBRP))
    return NULL;
