...
StringDictionaryHTFC::StringDictionaryHTFC(IteratorDictString *it,
                                           uint bucketsize) {
  this->type = HTFC;
  ...
  for (size_t i = 0; i < dict->bytesStrings; i++)
    freqs[(int)(dict->textStrings[i])]++;
  ...
  // 3) Compressing the dictionary and building the decoding table
  {
    ...
    for (uint current = 1; current <= elements; current++) {
      ...
      if (((current - 1) % bucketsize) == 0) {
        ...
        do {
          ...
          ptr++;
          builder->insertDecodeableSubstr(symbol, &codeSubstr, &ptrSubstr,
        ...
        if (textSubstr.size() > 0) {
          ...
          if (ptrSubstr > TABLEBITSO) {
            ...
            while (TABLEBITSO > ptrSubstr) {
              {
                // The VByte value is independently encoded to avoid confusions
                // due to '\0' bytes
                uint value = 0;
          ...
          builder->insertEndingSubstr(&codeSubstr, &ptrSubstr, &textSubstr,
                                      &lenSubstr);
        }
        ...
        for (uint i = 0; i < xptr; i++) {
          ...
          bytesStrings +=
              coder->encodeSymbol(code, &(textStrings[bytesStrings]), &offset);
          ptr++;

          builder->insertDecodeableSubstr(code, &codeSubstr, &ptrSubstr,
                                          &textSubstr, &lenSubstr);
        }
        ...
        do {
          // Encoding the string
          uchar symbol = (dict->textStrings[ptr]);
          bytesStrings += coder->encodeSymbol(
    ...
    maxcomplength +=
        4; // The value is increased because advanced readings in decoding...
  }
...
uint StringDictionaryHTFC::locate(uchar *str, uint strLen) {
  uint id = NORESULT;
...
uchar *StringDictionaryHTFC::extract(size_t id, uint *strLen) {
  if ((id > 0) && (id <= elements)) {
...
IteratorDictID *StringDictionaryHTFC::locatePrefix(uchar *str, uint strLen) {
  // Encoding the string
  uint encLen, offset;
  ...
  if (leftBucket > NORESULT) {
    ...
    if (leftBucket == rightBucket) {
      ...
      if (leftID == NORESULT)
        ...
        rightID =
            leftID +
            searchDistinctPrefix(&c, scanneable - leftID + 1, str, strLen) - 1;
...
IteratorDictID *StringDictionaryHTFC::locateSubstr(uchar *str, uint strLen) {
  cerr << "This dictionary does not provide substring location" << endl;
  return NULL;
}

uint StringDictionaryHTFC::locateRank(uint rank) { return rank; }

IteratorDictString *StringDictionaryHTFC::extractPrefix(uchar *str,
                                                        uint strLen) {
  ...
  if (left != NORESULT) {
    ...
    return new IteratorDictStringHTFC(
        table, codewords, textStrings, blStrings, leftbucket, leftpos,
        bucketsize, right - left + 1, maxlength, maxcomplength);
  } else
    return NULL;
}

IteratorDictString *StringDictionaryHTFC::extractSubstr(uchar *str,
                                                        uint strLen) {
  cerr << "This dictionary does not provide substring extraction" << endl;
  return 0;
}

uchar *StringDictionaryHTFC::extractRank(uint rank, uint *strLen) {
  return extract(rank, strLen);
}

IteratorDictString *StringDictionaryHTFC::extractTable() {
  return new IteratorDictStringHTFC(table, codewords, textStrings, blStrings, 1,
                                    0, bucketsize, elements, maxlength,
                                    maxcomplength);
}

size_t StringDictionaryHTFC::getSize() {
  return bytesStrings * sizeof(uchar) + blStrings->getSize() +
         256 * sizeof(Codeword) + table->getSize() +
...
uchar *StringDictionaryHTFC::getHeader(size_t idbucket) {
...
ChunkScan StringDictionaryHTFC::decodeHeader(size_t idbucket) {
  uchar *ptr = textStrings + blStrings->getField(idbucket);
  ChunkScan chunk = {
      0, 0, ptr, maxcomplength, new uchar[4 * maxlength + table->getK()],
      0, 0, 1};
...
bool StringDictionaryHTFC::locateBucket(uchar *str, uint strLen,
                                        size_t *idbucket) {
...
void StringDictionaryHTFC::locateBoundaryBuckets(uchar *str, uint strLen,
  ...
  while (*left <= *right) {
    ...
    if (offset != 0)
      header[strLen - 1] = header[strLen - 1] & cmask;
    cmp = memcmp(header, str, strLen);
...
uint StringDictionaryHTFC::searchPrefix(ChunkScan *c, uint scanneable,
...
uint StringDictionaryHTFC::searchDistinctPrefix(ChunkScan *c, uint scanneable,
                                                uchar *str, uint strLen) {
  uint id = 1;

  for (id = 1; id < scanneable; id++)
    if (coder->decodeString(c) < strLen)
      break;

  return id;
}

void StringDictionaryHTFC::resetScan(ChunkScan *c, size_t idbucket) {
  c->c_chunk = 0;
  c->c_valid = 0;
