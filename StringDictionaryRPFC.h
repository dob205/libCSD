...
public:
  ...
  */
  static StringDictionary *load(std::istream &in);
...
protected:
  ...
  uchar *textStrings; //! Hu-Tucker over the Front-Coding representation of all
                      //! remaining strings
  ...
  */
  inline uint searchPrefix(uchar **ptr, uint scanneable, uchar *decoded,
                           uint *decLen, uchar *str, uint strLen, uint *offset);
  ...
  /** Searches the first string non-prefixed for the given one.
      ...
      @returns the internal ID (of 0 if it is not in the bucket).
  */
  inline uint searchDistinctPrefix(uchar *ptr, uint scanneable, uchar *decoded,
