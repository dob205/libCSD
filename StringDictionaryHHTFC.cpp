...
#include "iterators/IteratorDictStringHHTFC.h"
...
StringDictionaryHHTFC::StringDictionaryHHTFC(IteratorDictString *it,
  ...
  while (bucket <= dict->buckets) {
    ...
    for (; pbeg < pend; pbeg++)
      freqsHU[(int)(dict->textStrings[pbeg])]++;
  }
    ...
    for (uint current = 1; current <= elements; current++) {
      ...
      if (((current - 1) % bucketsize) == 0) {
        ...
        if (textSubstr.size() > 0) {
          ...
          builderHT->insertEndingSubstr(&codeSubstr, &ptrSubstr, &textSubstr,
                                        &lenSubstr);
        }
        ...
        for (uint i = 0; i < xptr; i++) {
          ...
          builderHU->insertDecodeableSubstr(code, &codeSubstr, &ptrSubstr,
                                            &textSubstr, &lenSubstr);
        }
        ...
        do {
          ...
          builderHU->insertDecodeableSubstr(symbol, &codeSubstr, &ptrSubstr,
                                            &textSubstr, &lenSubstr);
        } while (dict->textStrings[ptr - 1] != '\0');
        ...
        if ((current % bucketsize) == 0) {
          ...
          // Adding an ending decodeable string (if required)
          if (textSubstr.size() > 0)
            builderHU->insertEndingSubstr(&codeSubstr, &ptrSubstr, &textSubstr,
                                          &lenSubstr);
...
uchar *StringDictionaryHHTFC::extract(size_t id, uint *strLen) {
  if ((id > 0) && (id <= elements)) {
...
IteratorDictID *StringDictionaryHHTFC::locateSubstr(uchar *, uint) {
...
uchar *StringDictionaryHHTFC::extractRank(uint rank, uint *strLen) {
  return extract(rank, strLen);
}

IteratorDictString *StringDictionaryHHTFC::extractTable() {
  return new IteratorDictStringHHTFC(tableHT, tableHU, codewordsHT, textStrings,
                                     blStrings, 1, 0, bucketsize, elements,
                                     maxlength, maxcomplength);
}

size_t StringDictionaryHHTFC::getSize() {
  return bytesStrings * sizeof(uchar) + blStrings->getSize() +
         256 * 2 * sizeof(Codeword) + tableHT->getSize() + tableHU->getSize() +
         sizeof(StringDictionaryHHTFC);
}

void StringDictionaryHHTFC::save(std::ofstream &out) {
...
StringDictionary *StringDictionaryHHTFC::load(std::ifstream &in) {
...
uint StringDictionaryHHTFC::searchDistinctPrefix(ChunkScan *c, uint scanneable,
  ...
  for (id = 1; id < scanneable; id++) {
    if (coderHU->decodeString(c) < strLen)
      break;
  }
